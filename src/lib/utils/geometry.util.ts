...
export class GeometryUtil {
	 ...
	 */
	static getDistanceFromCenter(point: Point, center: Point): number {
	 ...
	 */
	static getAngleFromCenter(point: Point, center: Point, rotationOffset: number = 0): number {
		const dx = point.x - center.x;
		const dy = point.y - center.y;
	...
	static describeArc(
		...
		const outerStart = this.polarToCartesian(centerX, centerY, outerRadius, startAngle);
		const outerEnd = this.polarToCartesian(centerX, centerY, outerRadius, endAngle);
	 ...
	 */
	static getSegmentFromAngle(angle: number, segmentAngle: number): number {
